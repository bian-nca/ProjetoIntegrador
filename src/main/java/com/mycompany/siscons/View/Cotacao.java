...
import Reports.RelatorioCotacao;
...
public class Cotacao extends javax.swing.JFrame {
    ...
    public Cotacao() throws SQLException {
        try {
        ...
        String sql = "SELECT MAX(IDCOTACAO) AS MAX_IDCOTACAO FROM COTACAO"; //estou obtendo o ultimo valor da coluna "idvenda" da minha tablea "vendas" e irei incrementá-lo posteriormente em +1
        Connection con = SQLConection.getConnection();
        PreparedStatement stmt = con.prepareStatement(sql);
        ResultSet rs = stmt.executeQuery(sql);
        if (rs.next()) {
            int maxIdVenda = rs.getInt("MAX_IDCOTACAO");
            int proxIdVenda = maxIdVenda + 1;
            txt_pesquisa.setText(Integer.toString(proxIdVenda));
            idCotacaoAtual = proxIdVenda;
        }
    }
    ...
    private void initComponents() {
        ...
        btn_confirmar.setText("IMPRIMIR");
        ...
        jLabel8.setText("Data da Cotação:");
        ...
        getContentPane().add(txt_pesquisa, new org.netbeans.lib.awtextra.AbsoluteConstraints(20, 20, -1, -1));
    ...
    private void btn_confirmarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btn_confirmarActionPerformed
        ...
        {
            try {
                String idcotacao = txt_pesquisa.getText();
                ...
                SQLConection conection = new SQLConection();
                String sql = "INSERT INTO COTACAO(idcotacao, datavenda, id_cliente, nome_cli, rua, bairro, numero, cidade, id_vendedor, nomevend, valor_wth_desc, desconto, vlr_total, cancelada, faturada) VALUES ('"+idcotacao+"', '"+data+"','"+clienteid+"', '"+clientenome+"', '"+enderecocli+"', '"+bairro+"', '"+numero+"', '"+cidade+"', '"+idvend+"', '"+nomevend+"', '"+valor_unit+"', '"+descontinho+"','"+vlr_total+"','"+situation+"', '"+fatu+"')";
                conection.SqlExecution(sql);
                JOptionPane.showMessageDialog(null, "COTAÇÃO SALVA EM C://");
                this.dispose();
            } catch (SQLException ex) {
                JOptionPane.showMessageDialog(null, "NÃO FOI POSSÍVEL ESTABELECER CONEXÃO COM O BANCO DE DADOS");
            }
            
            RelatorioCotacao relatorioCotacao = new RelatorioCotacao();
            try {
                relatorioCotacao.criarRelatorioCotacao(idCotacaoAtual);
            } catch (SQLException ex) {
    ...
    private void jButton2ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton2ActionPerformed
        ...
        bairrocli.setText("");
        txt_ncli1.setText("");
        txt_cidadecli.setText("");
        txt_codvend.setText("");
        txt_nomevend.setText("");
        valor_tot.setText("");
        desconto.setText("");
        txt_tot.setText("");
    ...
    private void botao_addActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_botao_addActionPerformed
        ...
        if(botao_add.isEnabled()) {  
            if(qtd_vend <= qtd_estoq) { try {
                //está validando se o usuario está inserindo uma quantidade maior do que o saldo existente no produto.
                String idCotacao = Integer.toString(idCotacaoAtual);
                String codProduto = codigo_produto.getText();
                String descProduto = produto_descricao.getText();
                String qtdProduto = qtd_vendas.getText();
                String valorUnit = vlr_unitario.getText();
                String valorTotal = totalizando.getText();
                DefaultTableModel modelo = (DefaultTableModel) tabela_vendas.getModel();          
                modelo.addRow(new Object[]{codProduto, descProduto, qtdProduto, valorUnit, valorTotal});
                int subt = qtd_estoq - qtd_vend;

                SQLConection conection = new SQLConection();
                String sql = "INSERT INTO ITENS_COTACAO(iditens_cotacao, idcotacao, codproduto, descproduto, qtdproduto, valorunit, valortotal) VALUES (null, '"+idCotacao+"','"+codProduto+"', '"+descProduto+"', '"+qtdProduto+"', '"+valorUnit+"', '"+valorTotal+"')";
                conection.SqlExecution(sql);
                } catch (SQLException ex) {
                    JOptionPane.showMessageDialog(null, "ERRO SQL AO SALVAR ITEM");
            }
    ...
    private void botao_rmvActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_botao_rmvActionPerformed
        ...
        if(botao_rmv.isEnabled()){
            ...
            SQLConection conection = new SQLConection();
            String sqlDelete = "DELETE FROM itens_cotacao WHERE codproduto = '"+codigo_produto.getText()+"' AND idcotacao = '"+idCotacaoAtual+"'";
            try {
                conection.SqlExecution(sqlDelete);
                JOptionPane.showMessageDialog(null,"item excluido da cotação");
            } catch (SQLException ex) {
